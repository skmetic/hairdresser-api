...
import { Validator, validate, IsDate } from 'class-validator';
...
export class WorkingHoursController {
  ...
  public async findWorkingHours(ctx: IRouterContext) {
    try {
      ...
      if (query.startDate && query.endDate && query.hairSalonId) {
        if (
          !this.validator.matches(query.startDate, Shared.DATE_REGEX) ||
          !this.validator.matches(query.endDate, Shared.DATE_REGEX) ||
  ...
  public async findWorkingHoursByDayAndHairSalon(ctx: IRouterContext) {
    try {
      const query = ctx.query;
      if (query.date && query.hairSalonId) {
        if (
          !this.validator.matches(query.date, Shared.DATE_REGEX) ||
          !this.validator.isNumberString(query.hairSalonId)
        ) {
          throw new Error(ErrorMessages.INVALID_PARAMS);
        }
        const result = await this.workingHoursService.findWorkingHoursByDayAndHairSalon(
          query.date,
          parseInt(query.hairSalonId)
        );
        ctx.body = result;
